...
class ExtractorMetric(Metric):
    ...
    def __call__(self, pred_spans, gold_spans):
        ...
        for idx in range(batch_size):
            ...
            self.c_fp += c_fp
            self.c_fn += c_fn
            self.span_count+=gold_span_num
            self.correct_span+=c_tp
    ...
    def get_metric(self, reset=False):
        ...
        acc=self.correct_span/self.span_count
        if reset:
            self.reset()
        return i_p, i_r, i_f, c_p, c_r, c_f,acc
...
class TriggerMetric(Metric):
    ...
    def get_metric(self, reset):
        i_p, i_r, i_f, c_p, c_r, c_f,acc = self.metric.get_metric()
        if reset:
            self.metric.reset()
        return {"t_i_p": i_p, "t_i_r": i_r, "t_i_f": i_f,
                "t_c_p": c_p, "t_c_r": c_r, "t_c_f": c_f,"acc":acc}
...
class ArgumentMetric(Metric):
    ...
    def get_metric(self, reset):
        ...
        return {"r_i_p": i_p, "r_i_r": i_r, "r_i_f": i_f,
                "r_c_p": c_p, "r_c_r": c_r, "r_c_f": c_f,"acc":acc}
