    ...
    def forward(self, sentence, sentence_id, type_ids, event_type, trigger=None, roles=None):
        ...
        if roles is not None: # 训练过程
            ...
            if embedding.is_cuda:
                start_role_label = start_role_label.cuda(tokens.device)
                end_role_label = end_role_label.cuda(tokens.device)
            for idxb in range(B):
            ...
            for idc in range(self.role_num):
                ...
                if embedding.is_cuda:
                    batch_weight = batch_weight.cuda(tokens.device)
                ...
                if self.training:
                    ...
                    if embedding.is_cuda:
                        tagger_ief = tagger_ief.cuda(tokens.device)
                        batch_af = batch_af.cuda(tokens.device)
...
class TriggerExtractor(Model):
    ...
    def forward(self, sentence, sentence_id, triggers=None, origin_text=None):
        ...
        if triggers is not None: # trian 过程
            ...
            if embedding.is_cuda:
                trigger_label = trigger_label.cuda(tokens.device)
            for idxb in range(B):
