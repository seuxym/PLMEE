...
import datetime
import traceback
from typing import Callable, Dict, Optional, List, Tuple, Union, Iterable, Any
...
class Trainer(TrainerBase):
    def __init__(self,
                 ...
                 batch_forward_cb:Callable=None,
                 epoch_start_cb:Callable=None,
                 epoch_end_cb:Callable=None,
        ...
        self.batch_forward_cb=batch_forward_cb
        self.epoch_end_cb=epoch_end_cb
        self.epoch_start_cb=epoch_start_cb
        self.logger=logger
        if patience is None:  # no early stopping
            if validation_dataset:
                self.logger.warning('You provided a validation dataset but patience was set to None, '
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        """
        if self.epoch_start_cb!=None:
            self.epoch_start_cb(epoch)
        self.logger.info("Epoch %d/%d", epoch, self._num_epochs - 1)
        ...
        for batch_group in train_generator_tqdm:
            ...
            if self.batch_forward_cb!=None:
                metrics["batch_size"]=sum([training_util.get_batch_size(batch) for batch in batch_group])
                metrics["batch_index"]=batches_this_epoch
                metrics["batch_total"]=num_training_batches
                self.batch_forward_cb(training=True,metrics=metrics)
        metrics = training_util.get_metrics(self.model, train_loss, batches_this_epoch, reset=True)
        metrics['cpu_memory_MB'] = peak_cpu_usage
        
        for (gpu_num, memory) in gpu_usage:
            metrics['gpu_'+str(gpu_num)+'_memory_MB'] = memory
        if self.epoch_end_cb!=None:
            self.epoch_end_cb()
        return metrics
    ...
    def _validation_loss(self) -> Tuple[float, int]:
        ...
        for batch_group in val_generator_tqdm:
            ...
            if self.batch_forward_cb!=None:
                val_metrics["batch_size"]=sum([training_util.get_batch_size(batch) for batch in batch_group])
                val_metrics["batch_index"]=batches_this_epoch
                val_metrics["batch_total"]=num_validation_batches
                self.batch_forward_cb(training=False,metrics=val_metrics)
