...
from dueereader import DataMeta, TriggerReader, RoleReader, ETRoleReader, ETTextReader, CustomSpanField, TextReader

device = torch.device(1 if torch.cuda.is_available() else 'cpu')   # 设备
device_num = -1
...
parser.add_argument('--pretrained_bert', type=str,
                    default='BertPara/pytorch/chinese_roberta_wwm_ext')
parser.add_argument('--bert_vocab', type=str,
                    default='BertPara/pytorch/chinese_roberta_wwm_ext/vocab.txt')

parser.add_argument('--save_trigger_dir', type=str,
                    default='./save/DuEE/bert_large/trigger/model_state_epoch_27.th')
parser.add_argument('--save_role_dir', type=str,
                    default='./save/DuEE/bert_large/role/model_state_epoch_29.th')

parser.add_argument('--data_meta_dir', type=str, default='./data/DuEE')
parser.add_argument('--extractor_train_file', type=str,
                    default='./data/DuEE/train.json')
parser.add_argument('--extractor_val_file', type=str,
                    default='./data/DuEE/dev.json')
parser.add_argument('--extractor_test_file', type=str)
parser.add_argument('--extractor_batch_size', type=int, default=28)
parser.add_argument('--extractor_argument_prob_threshold',
                    type=float, default=0.5)
parser.add_argument("--extractor_cuda_device", type=int, default=0)
parser.add_argument('--task_name', type=str, default='DuEE',
                    help="DuEE or DuEE-Fin")  # DuEE or DuEE-Fin

# True: trigger + argument方式; False: ET + text -> argument方式
parser.add_argument('--istrigger', action="store_true", default=True)
# 在上述为False时, True: ETid + text, False: ETText + text
parser.add_argument('--isETid', action="store_true")

args = parser.parse_args()
device = torch.device(args.extractor_cuda_device)
device_num = args.extractor_cuda_device
# trigger 提取步骤
...
def trigger_extractor_deal(pre_dataset, iterator, trigger_model_path, dataset_meta):
    def get_instance(sentence_data, t_list):
        ...
        for trigger in t_list:
            ...
            trigger_span_field = CustomSpanField(
                trigger_span_start, trigger_span_end, et_id, -1)
...
def argument_extractor_deal(instances, iterator, argument_model_path, dataset_meta):
    ...
    for data in iterator(instances, num_epochs=1):
        ...
        roles = data['roles']
        output = argument_extractor(
            sentence, sentence_id, type_ids, event_type, trigger)
        batch_spans = argument_extractor.metric.get_span(
            output['start_logits'], output['end_logits'], event_type)
...
def argument_et_extractor_deal(instances, iterator, argument_model_path, dataset_meta):
    ...
    for data in iterator(instances, num_epochs=1):
        ...
        event_type = data['event_type']
        output = argument_extractor(
            sentence, sentence_id, type_ids, event_type)
        batch_spans = argument_extractor.metric.get_span(
            output['start_logits'], output['end_logits'], event_type)
...
if __name__ == "__main__":
    ...
    data_meta = DataMeta(event_id_file=args.data_meta_dir +
                         "/events.id", role_id_file=args.data_meta_dir + "/roles.id")
    ...
    if args.istrigger:  # 原版PLMEE trigger + role 方式
        ...
        trigger_reader = TriggerReader(
            data_meta=data_meta, token_indexer=bert_indexer)
        ...
        if not os.path.exists(instance_pkl_path):
            instances = trigger_extractor_deal(
                pre_dataset=pre_dataset, iterator=iterator, trigger_model_path=trigger_model_path, dataset_meta=data_meta)
        ...
        if not os.path.exists(result_pkl_path):
            pred_spans = argument_extractor_deal(
                instances=instances, iterator=iterator, argument_model_path=argument_model_path, dataset_meta=data_meta)
        ...
        # 加载train 和 val 数据集 用来填充data_meta中的计数模块 (所以采用何种reader在该环节都不影响)
        # 后续可更改为: 1、et与rt关系从schema中读取   2、role权重用二进制文件在训练时进行保存的形式
        role_reader = ETRoleReader(
            data_meta=data_meta, token_indexer=bert_indexer, isETid=args.isETid)
        role_train_dataset = role_reader.read(args.extractor_train_file)
        role_val_dataset = role_reader.read(args.extractor_val_file)

        data_meta.compute_AF_IEF(role_train_dataset)  # 根据AF_IEF来计算role的重要程度
        text_reader = ETTextReader(
            data_meta=data_meta, token_indexer=bert_indexer, isETid=args.isETid)
        pre_dataset = text_reader.read(args.extractor_test_file)  # 用来预测的文件
        print('=====> Extracting arguments...')
        if not os.path.exists(result_pkl_path):
            pred_spans = argument_et_extractor_deal(
                instances=pre_dataset, iterator=iterator, argument_model_path=argument_model_path, dataset_meta=data_meta)
            pkl.dump(pred_spans, open(result_pkl_path, 'wb'))
        else:
            pred_spans = pkl.load(open(result_pkl_path, 'rb'))
        ...
        for sid, pred_span in pred_spans.items():
            ...
            for ids, span in enumerate(pred_span):
                ...
                if args.istrigger:  # PLMEE 原版
                    ...
                    if args.isETid:
                        argument = text[span[0]-1-1: span[1] + 1 -
                                        len(e_dict['event_type'])-1]  # etid + text
                    else:
                        # et + text
                        argument = text[span[0]-len(e_dict['event_type']) -
                                        1: span[1] + 1-len(e_dict['event_type'])-1]
